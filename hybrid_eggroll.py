"""
Hybrid EGGROLL: Parallel Semantic Repair (v5)
=============================================
Architecture: Transformer Encoder -> Latent Semantic Slots -> Parallel Graph Repair -> Transformer Decoder
"""
...
class ModelConfig:
    def __init__(self, h, L_enc, L_rep, L_dec, rep_steps):
        self.h = h; self.L_enc = L_enc; self.L_rep = L_rep; self.L_dec = L_dec; self.rep_steps = rep_steps
...
def init_params(key, cfg):
    ...
    for i in range(cfg.L_enc):
        p[f"e.{i}.qkv"] = W(3*h, h); p[f"e.{i}.o"] = W(h, h); p[f"e.{i}.f1"] = W(2*h, h); p[f"e.{i}.f2"] = W(h, 2*h)
    for i in range(cfg.L_rep):
        p[f"r.{i}.w"] = W(h, 5*h); p[f"r.{i}.b"] = jnp.zeros(h)
    for i in range(cfg.L_dec):
        p[f"d.{i}.sqkv"] = W(3*h, h); p[f"d.{i}.so"] = W(h, h); p[f"d.{i}.cq"] = W(h, h); p[f"d.{i}.ckv"] = W(2*h, h); p[f"d.{i}.co"] = W(h, h); p[f"d.{i}.f1"] = W(2*h, h); p[f"d.{i}.f2"] = W(h, 2*h)
    return p
...
def rollout(params, tokens, target_seq, cfg):
    ...
    for i in range(cfg.L_enc):
        qkv = h @ params[f"e.{i}.qkv"].T; q, k, v = jnp.split(qkv, 3, -1); a = jax.nn.softmax((q @ k.swapaxes(1,2)) * scale, -1)
        h = rms_norm(h + (a @ v) @ params[f"e.{i}.o"].T); h = rms_norm(h + jnp.tanh(h @ params[f"e.{i}.f1"].T) @ params[f"e.{i}.f2"].T)
    enc = h; sq = params["sq"] + params["st"]; a = jax.nn.softmax((sq[None,:,:] @ enc.swapaxes(1,2)) * scale, -1); slots = rms_norm(a @ enc + sq[None,:,:])
    def rep_step(s, _):
...
def loss_fn(params, batch, cfg, backprop=True):
    ...
    t_acc = jnp.mean((jnp.argmax(logits, -1) == y) * mask) / jnp.mean(mask); r_acc = jnp.mean(jnp.argmax(logits[:, 10], -1) == y[:, 10])
...
def egg_step(params, key, cfg, sigma, lr, pop):
    ...
    target_names = [n for n in params.keys() if any(x in n for x in ("sq", "st", "Wb", "bb", "Wp", "bp", "r.", "be", "pe"))]
    def df(pk):
        def sc(s):
            nt = jax.tree_util.tree_map(lambda p: jax.random.normal(pk, p.shape), params)
            nt_masked = {n: (nt[n] if n in target_names else jnp.zeros_like(nt[n])) for n in params}
            pn = jax.tree_util.tree_map(lambda p, n: p + s * sigma * n, params, nt_masked)
            l, ax = loss_fn(pn, batch, cfg, False); return -l, nt_masked
        s1, n = sc(1.0); s2, _ = sc(-1.0); return (s1-s2), n
    dfs, ns = vmap(df)(p_keys)
    def up(p, n):
        grad = jnp.mean(n * dfs[(...,) + (None,)*(n.ndim-1)], 0); return p + (lr / sigma) * grad
    params = jax.tree_util.tree_map(up, params, ns); _, aux = loss_fn(params, batch, cfg, False); return params, aux

def main():
    p = argparse.ArgumentParser(); p.add_argument("--h", type=int, default=16); p.add_argument("--bp", type=int, default=500); p.add_argument("--egg", type=int, default=500); p.add_argument("--cpu", action="store_true"); args = p.parse_args()
    ...
    print(f"Hybrid EGGROLL v5: {args.h}h. Platform: {jax.default_backend()}"); print("Phase 1: Backprop..."); start = time.time()
    for i in range(1, args.bp + 1):
        params, opt, l, aux = train_step(params, opt, jax.random.fold_in(key, i), cfg, 1e-3, i)
        if i % 100 == 0 or i == 1: print(f"  BP {i:4d} | loss {l:5.3f} | tok_acc {aux[0]:.3f} | ref_acc {aux[1]:.3f} | {time.time()-start:.1f}s")
    if args.egg > 0:
        print("\nPhase 2: EGGROLL..."); start = time.time()
        for i in range(1, args.egg + 1):
            params, aux = egg_step(params, jax.random.fold_in(key, i+1000), cfg, 0.2, 0.02, 32)
            if i % 100 == 0 or i == 1: print(f"  EGG {i:4d} | tok_acc {aux[0]:.3f} | ref_acc {aux[1]:.3f} | {time.time()-start:.1f}s")
    def tn(t, cfg):
    ...
    print(f"  In:  {' '.join(tn(int(t), cfg) for t in tk[0])}"); print(f"  Target: {' '.join(tn(int(t), cfg) for t in tg[0, 1:])}")
    print(f"  Pred:   {' '.join(tn(int(t), cfg) for t in jnp.argmax(logits[0], -1))}")
